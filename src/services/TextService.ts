...
export const fetchTitles = (): Promise<AxiosResponse<Array<Text>>> => {
    return axios.get<Array<Text>>(BASE_URL, getBasicAuthenticationConfig());
};

export const fetchText = (id: string): Promise<AxiosResponse<Text>> => {
    return axios.get<Text>(`${BASE_URL}/${id}`, getBasicAuthenticationConfig());
};

export const fetchParsedText = (id: string): Promise<AxiosResponse<ParsedText>> => {
    return axios.get<ParsedText>(`${BASE_URL}/${id}/parsed`, getBasicAuthenticationConfig());
};

export const addTextToDB = (text: Text, callback?: () => void): void => {
    axios.post(BASE_URL, text, getBasicAuthenticationConfig())
        .then(() => {
            callback && callback();
        })
...
export const updateTextInDB = (text: Text, callback?: () => void): void => {
    axios.put(BASE_URL, text, getBasicAuthenticationConfig())
        .then(() => {
            callback && callback();
        })
        .catch(err => console.error(err.response.data));
